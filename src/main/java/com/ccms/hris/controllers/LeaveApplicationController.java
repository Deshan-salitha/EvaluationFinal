...
public class LeaveApplicationController {
    ...
    public ResponseEntity createLeaveApplication(@RequestBody LeaveApplicationDto leaveApplicationDto) {
        System.out.println(leaveApplicationDto.toString());
        try {
    ...
    public ResponseEntity approveLeaveApplication(@RequestHeader(name="Authorization") String token,@RequestBody LeaveApplicationDto leaveApplicationDto){
        try {
//            LeaveApplication leaveApplication = new LeaveApplication();
//            leaveApplication.setLeaveRequestStatus(leaveApplicationDto.getLeaveRequestStatus());
            ...
            if (Objects.equals(user.getRoles().get(0).getName(), "ADMIN")){
                ...
                Optional<LeaveApplication> leaveApplication = leaveAppicationService.getLeaveApplicationById(leaveApplicationDto.getLeaveApplicationId());
                System.out.println("Leave Application : "+leaveApplication.toString());
                System.out.println("Requested User: "+requestedUser.toString());
                System.out.println("Approved: "+requestedUser.getLeaveAllocation());
                if (leaveApplication.isEmpty()){
                    return ResponseEntity.status(HttpStatus.OK).body(new ResponseWrapper(leaveApplicationDto, "Faild", "Application dosen't exist"));
                }else {
                    leaveApplication.ifPresent(existingLeaveApplication -> {
                        LeaveApplication application = existingLeaveApplication;
                        application.setLeaveRequestStatus(leaveApplicationDto.getLeaveRequestStatus());
                        if (leaveApplicationDto.getLeaveRequestStatus().equals("APPROVED")){
//                            requestedUser.setLeaveAllocation();
                            requestedUser.setLeaveAllocation(requestedUser.getLeaveAllocation() );
                        }
                        System.out.println("status Updated : "+application.toString());
                        try {

                            leaveAppicationService.UpdateApplicationRequestStatus(application);
                        } catch (Exception e) {
                            throw new RuntimeException(e);
                        }
                        //operate on existingCustomer
                    });
                    return ResponseEntity.status(HttpStatus.OK).body(new ResponseWrapper(leaveApplicationDto, "Success", "Application dose exist"));
                }
...
//    public ResponseEntity rejectLeaveApplication(@RequestHeader(name="Authorization") String token,@RequestBody LeaveApplicationDto leaveApplicationDto){
//        try {
////            LeaveApplication leaveApplication = new LeaveApplication();
////            leaveApplication.setLeaveRequestStatus(leaveApplicationDto.getLeaveRequestStatus());
//            System.out.println(token.split(" ")[1]);
...
//                    return ResponseEntity.status(HttpStatus.OK).body(new ResponseWrapper(leaveApplicationDto, "Faild", "Application dosen't exist"));
//                }else {
//                    return ResponseEntity.status(HttpStatus.OK).body(new ResponseWrapper(leaveApplicationDto, "Faild", "Application dose exist"));
//                }
////                leaveAppicationService.approveApplication(leaveApplication);
//            }else {
//                return ResponseEntity.status(HttpStatus.OK).body(new ResponseWrapper(leaveApplicationDto, "Faild", "You can't access this"));
